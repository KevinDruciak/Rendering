...
GlobalSceneData::GlobalSceneData( void ) : shader(NULL), lightsOn(true) {}
...
namespace Ray
{
	istream &operator >> ( istream &stream , File &file )
	{
		...
		if( !_stream ) THROW( "Failed to open file for reading: %s" , filename.c_str() );
...
void Scene::drawOpenGL( void ) const
{
	...
	} else
	{
		for (int i = 0; i < _globalData.lights.size(); i++) {
			if (_globalData.lights[i]->name() == "directional light") {
				_globalData.lights[i]->drawOpenGL(i, _globalData.shader ? _globalData.shader->glslProgram : NULL);
			}
			else {
				_globalData.lights[i]->drawOpenGLoff(i, _globalData.shader ? _globalData.shader->glslProgram : NULL);
			}
		}
	}
	...
	for( int j=0 ; j<height ; j++ )
	{
		for( int i=0 ; i<width ; i++ )
		{
			try
			{
				...
				Ray3D ray = _globalData.camera.getRay( i , height-j-1 , width , height, 1 );
				Point3D c = getColor( ray , rLimit , Point3D( cLimit , cLimit , cLimit ), this->_globalData.lightsOn );
				Pixel32 p;
				p.r = (int)(c[0]*255);
				p.g = (int)(c[1]*255);
				p.b = (int)(c[2]*255);
...
double Scene::intersect( Util::Ray3D ray , RayShapeIntersectionInfo &iInfo , BoundingBox1D range , std::function< bool (double) > validityLambda ) const
{
